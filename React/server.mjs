...
const publicPath = '/intelligent-forms-doc-generation/react';
const distPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'dist');
const indexFile = path.join(distPath, 'index.html');

// Fail fast when the build output is missing (e.g. server-side build skipped).
if (!fs.existsSync(indexFile)) {
  console.error(
    `Build output missing: ${indexFile}. Run 'npm run build' before starting the server.`
  );
  process.exit(1);
}
